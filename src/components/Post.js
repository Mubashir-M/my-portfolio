  ...
  return (
    <main className = 'bg-gray-100 min-h-screen p-12'>
      <section className = 'container mx-auto'>
        <h1 className = 'text-5xl flex justify-center font-TT'>Blog Post Page</h1>
              ...
              <Link to = {'/post/'+ post.slug.current} key = {post.slug.current}>
                <span className = 'block h-64 relative rounded shadow leading-snug bg-white border-l-8 border-gray-400' key = {index}>
                  ...
                  <span className = 'block relative h-full flex justify-end items-end pr-4 pb-4'>
                    <h3 className = 'text-white text-lg px-3 py-4 bg-gray-400 bg-opacity-75 rounded hover:bg-gray-800'>{post.title}</h3>
                  </span>
                </span>
              </Link>
            </article>
